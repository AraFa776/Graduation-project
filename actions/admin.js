...
export async function getVerifiedDoctors() {
  ...
  try {
    ...
    return { doctors: [], error: { code: "FETCH_FAILED" } };
  }
}
...
export async function approvePayout(formData) {
  ...
  try {
    ...
    const netEgp =
      payout.netAmountEgp ?? Math.round(Number(payout.netAmount) || 0);
    ...
    const { notifyPayoutProcessed } = await import("@/lib/notification-triggers");
    try {
      await notifyPayoutProcessed(payout, payout.doctor);
    } catch (e) {
      console.error("notifyPayoutProcessed:", e);
    }
